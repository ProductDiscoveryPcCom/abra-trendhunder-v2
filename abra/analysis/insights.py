...
def calculate_changes(timeline_data):
    """
    Calcula cambios temporales. SIEMPRE devuelve números, nunca None.
    """
    # Valores por defecto seguros
    default_return = (0.0, 0.0, 0.0, 0.0)
    ...
    try:
        ...
        for p in values:
            if p.get('values'):
                val = p['values'][0].get('extracted_value', 0)
        ...
        # Valores seguros
        current = float(all_values[-1]) if all_values else 0.0
        month_ago = float(all_values[-5]) if len(all_values) >= 5 else float(all_values[0])
        quarter_ago = float(all_values[-13]) if len(all_values) >= 13 else float(all_values[0])
        year_ago = float(all_values[-52]) if len(all_values) >= 52 else float(all_values[0])
        
        # Cálculos seguros
        month_change = ((current - month_ago) / month_ago * 100.0) if month_ago > 0 else 0.0
        quarter_change = ((current - quarter_ago) / quarter_ago * 100.0) if quarter_ago > 0 else 0.0
        year_change = ((current - year_ago) / year_ago * 100.0) if year_ago > 0 else 0.0
        avg_value = sum(all_values) / len(all_values) if all_values else 0.0
        
        # Asegurar que todo es float
        return (
            float(month_change),
            float(quarter_change),
            float(year_change),
            float(avg_value)
        )
...
def analyze_all_channels(brand, countries, categories, threshold):
    ...
    for geo in countries:
        ...
        with st.spinner(f'🌐 Analizando {brand} en {country_name} - Todos los canales...'):
            ...
            # Calcular métricas agregadas de todos los canales (robusto contra None)
            all_month_changes = []
            all_quarter_changes = []
            all_year_changes = []
            all_avg_values = []
            
            for ch in channel_results.values():
                if 'error' in ch:
                    continue
                    
                # Month change
                mc = ch.get('month_change', 0)
                if mc is not None and isinstance(mc, (int, float)):
                    all_month_changes.append(float(mc))
                    
                # Quarter change
                qc = ch.get('quarter_change', 0)
                if qc is not None and isinstance(qc, (int, float)):
                    all_quarter_changes.append(float(qc))
                    
                # Year change
                yc = ch.get('year_change', 0)
                if yc is not None and isinstance(yc, (int, float)):
                    all_year_changes.append(float(yc))
                    
                # Avg value (solo > 0)
                av = ch.get('avg_value', 0)
                if av is not None and isinstance(av, (int, float)) and av > 0:
                    all_avg_values.append(float(av))
            ...
            all_channels_data[geo] = {
                ...
                'month_change': sum(all_month_changes) / len(all_month_changes) if all_month_changes else 0.0,
                'quarter_change': sum(all_quarter_changes) / len(all_quarter_changes) if all_quarter_changes else 0.0,
                'year_change': sum(all_year_changes) / len(all_year_changes) if all_year_changes else 0.0,
                'avg_value': sum(all_avg_values) / len(all_avg_values) if all_avg_values else 0.0
            }
...
def consolidate_channel_data(channel_results, brand, geo):
    ...
    try:
        ...
        for channel_key, data in channel_results.items():
            ...
            if avg_val is not None and isinstance(avg_val, (int, float)) and avg_val > 0:
                consolidated['channels_with_data'] += 1
                
                # Volumen promedio del canal
                channel_volumes[channel_key] = float(avg_val)
                total_volume += float(avg_val)
        ...
        print(f"Error in consolidate_channel_data: {e}")
...
def generate_cross_channel_insights(channel_results, channel_volumes, dominant_channel):
    ...
    for channel_key, data in channel_results.items():
        ...
        if mc is not None and isinstance(mc, (int, float)) and mc > 10:
